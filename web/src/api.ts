...
export type Vehicle = {
  ...
  init_odometer: number; init_fuel: number; odometer: number; fuel_balance: number;
  active: number; notes: string;
  fuel_code?: string; fuel_name_uz?: string; fuel_name_ru?: string; unit_uz?: string; unit_ru?: string;
};

export type Driver = {
  id: number; full_name: string; tab_no: string; license_no: string;
  phone: string; class: string; active: number;
};

export type RouteRow = {
  id?: number; seq?: number; point_from: string; point_to: string;
  time_out: string; time_in: string; distance_km: number; cargo: string; cargo_ton: number;
};

export type Waybill = {
  id: number; number: string; date_from: string; date_to: string;
  vehicle_id: number; driver_id: number; status: 'draft' | 'issued' | 'closed';
  odo_start: number; odo_end: number | null; fuel_start: number; fuel_end: number | null;
  engine_hours: number; cargo_ton_km: number; winter: number;
  ...
  fuel_issued: number; distance_km: number | null;
